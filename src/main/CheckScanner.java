...
public class CheckScanner {
    ...
    private boolean hitByRook(int col, int row, Piece king, int kingCol, int kingRow, int colVal, int rowVal) {
        for (int i = 1; i < 10; i++) {
            ...
            if (piece != null && piece != board.selectedPiece) {
                if (!board.sameTeam(piece, king) && (piece.name.equals("Rook") || piece.name.equals("Queen") || piece.name.equals("Giraffe") || piece.name.equals("CombatVehicle") || piece.name.equals("Vizir"))) {
                    return true;
                }
                break;
            }
    ...
    private boolean hitByBishop(int col, int row, Piece king, int kingCol, int kingRow, int colVal, int rowVal) {
        for (int i = 1; i < 10; i++) {
            ...
            Piece piece = board.getPiece(kingCol - (i * colVal), kingRow - (i * rowVal));
            if (piece != null && piece != board.selectedPiece) {
                if (!board.sameTeam(piece, king) && (piece.name.equals("Bishop") || piece.name.equals("Queen") || piece.name.equals("Giraffe") || piece.name.equals("CombatVehicle") || piece.name.equals("Vizir"))) {
                    return true;
                }
                break;
            }
    ...
    private boolean checkKnight(Piece p, Piece k, int col, int row) {
        return p != null && !board.sameTeam(p, k) && (p.name.equals("Knight") || p.name.equals("Giraffe") || p.name.equals("CombatVehicle") || p.name.equals("Vizir") )&& !(p.col == col && p.row == row);
    }
